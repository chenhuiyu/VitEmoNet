...
LastEditTime: 2021-07-15 16:44:13
FilePath: \\SST-EmotionNet-main\\logger.py
'''
import os
import logging as py_logging

_log_level = {None: py_logging.NOTSET, "debug": py_logging.DEBUG, "info": py_logging.INFO, "warning": py_logging.WARNING, "error": py_logging.ERROR, "critical": py_logging.CRITICAL}
...
def get_logger(log_file_path=None, name="default_log", level=None):
    ...
    if (log_file_path is not None and not _check_file_handler(root_logger, log_file_path)):
        log_formatter = py_logging.Formatter("%(asctime)s [%(levelname)-5.5s] %(message)s", datefmt="%Y-%m-%d %H:%M:%S")
